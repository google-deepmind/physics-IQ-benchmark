...
class BaseTemplater:
    ...
    def from_csv(cls, df_path, use_action_suffix: bool = True)->"BaseTemplater":
        try:
            ...
            try:
                ...
                raise ValueError(f"Failed to read CSV with both ';' and default separators: {e}, {e2}")
    ...
    def get_subjectaction_description(self, identifier) -> str:
        ...
        action = self.data_field_handler(action, identifier, self.subjact_key, necessary=False)
        if pd.isna(action):
            raise ValueError(f"Subject-action description is missing for identifier {identifier}")
        if self.use_action_suffix:
            action_suffix = idenifier_data[self.subjactsuffix_key]
            action_suffix = self.data_field_handler(action_suffix, identifier, "subject-action post-fix", necessary=False)
            if action_suffix != "":
    ...
    def get_scene_description(self, identifier) -> str:
        ...
        return self.data_field_handler(data, identifier, self.scene_key, necessary=False)
    
    def get_scenesetup_description(self, identifier) -> str:
        data = self.get_identifier_data(identifier)[self.scenesetup_key]
        return self.data_field_handler(data, identifier, self.scenesetup_key, necessary=False)
    ...
    def generate_prompt(self, identifier) -> str:
        ...
        prompt = " ".join(filter(self.filter_empty, [self.camera_description, self.style_description, self.action_description, scenesetup_description, scene_description, subjectaction_description])) 
    ...
    def get_non_empty(self, key: str) -> dict:
        ...
        non_empty_identifiers = self.df[~self.df[key].isna()][self.identifier_key].tolist()
