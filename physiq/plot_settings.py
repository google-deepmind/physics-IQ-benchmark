...
_MODELS.append(
    Model(
        model_name="VideoPoet_multiframe",
        plotting_name="VideoPoet (multiframe)",
        color="#CC5500",
    )
)
_MODELS.append(
    Model(model_name="VideoPoet_i2v", plotting_name="VideoPoet (i2v)", color="#FF7F0E")
)
_MODELS.append(
    Model(
        model_name="Lumiere_multiframe",
        plotting_name="Lumiere (multiframe)",
        color="#1C69A7",
    )
)
_MODELS.append(
    Model(model_name="Lumiere_i2v", plotting_name="Lumiere (i2v)", color="#17BECF")
)
_MODELS.append(
    Model(model_name="Runway", plotting_name="Runway Gen 3 (i2v)", color="#2ca02c")
)
_MODELS.append(
    Model(model_name="Pika", plotting_name="Pika 1.0 (i2v)", color="#FFD700")
)
_MODELS.append(
    Model(
        model_name="SVD", plotting_name="Stable Video Diffusion (i2v)", color="#9467BD"
    )
)
_MODELS.append(Model(model_name="Sora", plotting_name="Sora (i2v)", color="#ff0606"))
_MODELS.append(Model(model_name="wan-i2v", plotting_name="Wan2.2", color="#4E79A7"))
_MODELS.append(Model(model_name="p-video", plotting_name="P-Video", color="#F28E2B"))
_MODELS.append(
    Model(model_name="hunyuan-video-v1", plotting_name="HunyuanV-1.5", color="#59A14F")
)
_MODELS.append(Model(model_name="sora-2", plotting_name="Sora 2", color="#E15759"))
_MODELS.append(
    Model(model_name="sora2new", plotting_name="Sora 2 (new)", color="#B07AA1")
)
_MODELS.append(
    Model(model_name="grok-imagine-video", plotting_name="Grok Video", color="#76B7B2")
)
_MODELS.append(
    Model(model_name="cosmos3-nano", plotting_name="Cosmos3-N", color="#DD31BA")
)
####################################################################
