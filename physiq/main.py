...
get_eval_type_from_run = lambda x: (
    x.split("__")[1] if "__" in x else (x.split("in_")[1] if "in_" in x else "?")
)
get_model_name = lambda x: max(
    (model for model in MODEL_NAMES if model in str(x)), key=len
)
...
if __name__ == "__main__":
    ...
    iq_tables = []
    filepaths = (
        [BASEPATH / subpath for subpath in subpaths]
        if USE_SUBPATHS
        else list(BASEPATH.glob("**/**/**/**/*.csv"))
    )
    filepaths.sort()
    ...
    for file_path in filepaths:
        ...
        model_name = max(
            (model for model in MODEL_NAMES if model in str(file_path)), key=len
        )
    ...
    pprint(
        ana_df[
            [EXPKEY, RUNKEY, FPS_KEY, ORIG_SCORE_KEY, VERIFIED_SCORE_KEY] + SCORES_LIST
        ]
    )
    pprint("\n\n")
    ...
    ana_df[EVAL_KEY] = ana_df[RUNKEY].apply(
        lambda x: (
            x.split("__")[1] if "__" in x else x.split("in_")[1] if "in_" in x else "?"
        )
    )
    # ana_df[SAMPLES_KEY] = ana_df[RUNKEY].apply(lambda x: "changed_samples" if "verified_in_original" in x else "original_samples")
    ana_df[PROMPT_KEY] = ana_df[RUNKEY].apply(
        lambda x: "op" if "op" in x else ("bpp" if "bpp" in x else "--")
    )
    ...
    df_g1[f"{col}_centered"] = df_g1.groupby("Model")[col].transform(
        lambda x: x - x.mean()
    )
    df_g2[f"{col}_centered"] = df_g2.groupby("Model")[col].transform(
        lambda x: x - x.mean()
    )
    ...
    df_sub[f"{col}_centered"] = df_sub.groupby(["Model"])[col].transform(
        lambda x: x - x.mean()
    )

    # Per-model SD per prompt condition
    sd_table = df_sub.groupby(["Model", "Prompt"])[col].std().unstack("Prompt")
    print(sd_table)
    ...
    pprint(
        ana_df.groupby([MODEL_KEY, EVAL_KEY, FPS_KEY, PROMPT_KEY]).mean(
            numeric_only=True
        )
    )
    ...
    pivot_df_meanstd = (
        ana_df.groupby([MODEL_KEY, EVAL_KEY, FPS_KEY, PROMPT_KEY])[
    ...
    if False:
        ...
        latex_table[EVAL_KEY] = pd.Categorical(
            latex_table[EVAL_KEY], categories=["verified", "original"], ordered=True
        )
        latex_table = latex_table.sort_values(
            [MODEL_KEY, EVAL_KEY, PROMPT_KEY]
        ).set_index([MODEL_KEY, EVAL_KEY, PROMPT_KEY])
        ...
        for line in lines:
            ...
            if in_data and "&" in line:
                if (
                    data_row_idx < len(row_evals)
                    and row_evals[data_row_idx] == "verified"
                ):
